...
def get_zest()
    ...
    cmd   "git submodule update --init"
    cmd   "git apply ../mruby-zest-build.patch"
...
def make_package_from_repos(demo_mode=true)
    ...
    cmd "sudo cp      ./mruby-zest-build/install-freebsd.sh /opt/zyn-fusion/"
    ...
    chdir "/opt/"
    cmd "sudo rm -f zyn-fusion-freebsd-64bit-#{CurrentVersion}-#{mode}.tar zyn-fusion-freebsd-64bit-#{CurrentVersion}-#{mode}.tar.bz2"
    cmd "sudo tar cf zyn-fusion-freebsd-64bit-#{CurrentVersion}-#{mode}.tar ./zyn-fusion"
    cmd "sudo bzip2 zyn-fusion-freebsd-64bit-#{CurrentVersion}-#{mode}.tar"
    chdir "#{this_dir}/"
    cmd   "sudo mv /opt/zyn-fusion-freebsd-64bit-#{CurrentVersion}-#{mode}.tar.bz2 ./"
    cmd   "sudo chown $(whoami) zyn-fusion-freebsd-64bit-#{CurrentVersion}-#{mode}.tar.bz2 || true"
end
